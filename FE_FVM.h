...
void FVM_1st(vector<vector<double>> &bounds,vector<vector<double>> &nodes,vector<vector<double>> &interiorFaces,vector<vector<double>> &u,vector<vector<double>> &B2E,vector<vector<double>> &Bn,vector<vector<double>> &In,int &nelem, double Minf){
    double CFL;
    int opt;
    int time;
...
timeStep: for (int t = 1; t < time; t++){
    ...
    residual = getRes(nelem,opt,u,interiorFaces,In,bounds,Bn,B2E,nodes,Minf);
